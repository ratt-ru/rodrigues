...
def simms(msname=None,label=None,tel=None,pos=None,pos_type='casa',
          ra='0h0m0s',dec='-30d0m0s',hours=4,dtime=10,freq0=700e6,
    ...
    if msname is None:
        ...
        msname = '%s/%s_%s_%dh%s_%s%s_%dch.MS'%(outdir,label or tel,dec_sign,hours,dtime,freq0,dfreq,nchan)
    ...
    fmt = 'msname="%(msname)s", label="%(label)s", tel="%(tel)s", pos="%(pos)s", '\
          'pos_type="%(pos_type)s", ra="%(ra)s", dec="%(dec)s", hours=%(hours).4g, '\
          'dtime="%(dtime)s", freq0="%(freq0)s", dfreq="%(dfreq)s", nchan=%(nchan)d, '\
          'stokes="%(stokes)s", start_time=%(start_time).4g, setlimits=%(setlimits)s, '\
...
if __name__=='__main__':
    ...
    simms(msname=args.name,label=args.label,tel=args.tel,pos=args.pos,
          pos_type=args.type,ra=args.ra,dec=args.dec,hours=args.synthesis,
